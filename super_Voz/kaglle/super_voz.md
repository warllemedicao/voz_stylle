...
4. **Constantes e Escopo:** Na transição modular, corrigimos o problema de escopo agrupando as constantes de ambiente como `HF_HUB_COMPAT_PACKAGE` dentro do `utils.py`. Além disso, em 09/06/2026, resolvemos o `NameError: restore_huggingface_subdir` consolidando a lógica de sincronização do F5-TTS no módulo `f5_integration.py` e corrigindo as importações internas entre os módulos do `runner_utils`.
