...
Antes do fine-tuning, o runner valida o checkpoint base configurado em `checkpoint_subpath`. Quando o arquivo e um `.safetensors` com pesos crus `transformer.*`, ele cria um checkpoint temporario compativel com o trainer em `ckpts/<dataset_name>/pretrained_*_ema.pt`, usando chaves `ema_model.transformer.*` e buffers `initted`/`step`. O diretorio `ckpts/<dataset_name>` tambem tem `pretrained_*` antigos removidos antes dessa etapa, evitando que o trainer reutilize um cache cru incompatível. Isso evita o erro `Missing key(s) ... ema_model.transformer` ao iniciar `accelerate launch`.
