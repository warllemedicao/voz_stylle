...
O Resemble Enhance e instalado com `--no-deps` para nao trocar o stack Torch/Torchaudio escolhido para a GPU do Kaggle. Por isso o runner precisa instalar manualmente os pins criticos declarados pelo proprio wheel do Resemble. Em especial, `enhance` depende de NumPy 1.26.x: com NumPy 2.x, o CFM interno falha em `float(scipy.optimize.fsolve(...))` com `only 0-dimensional arrays can be converted to Python scalars`. O bloco de limpeza agora instala `numpy==1.26.2`, `scipy==1.11.4`, `pandas==2.1.3`, `matplotlib==3.8.1`, `tabulate==0.8.10` e `resampy==0.4.2` antes de `resemble-enhance --no-deps`. Quando a limpeza iniciar, confira o log `[RESEMBLE][VERSOES]`; se aparecer `numpy=2...`, o ambiente ainda esta incompatível para `enhance`.

Instalacao de dependencia usada agora e fatal. O runner valida comandos de sistema (`ffmpeg`, `sox`, `espeak-ng`) e modulos Python essenciais depois do `pip install`; se algo nao instalar ou se um pin critico ficar diferente, o notebook aborta antes de iniciar a limpeza/treino. Isso vale para runtime ML, F5-TTS, limpeza/transcricao, DNSMOS/ONNX e Resemble. TeraBox e tentativas alternativas de restore/upload Hugging Face continuam tolerantes por serem persistencia opcional, nao bibliotecas do runtime.
...
Como o runner precisa importar `torch` para detectar a GPU antes do `pip install`, trocar o stack `torch/torchaudio/torchvision` no mesmo processo pode deixar modulos antigos em memoria. Isso causava falsos erros de validacao como `Could not load ... libtorchaudio.so` ou `operator torchvision::nms does not exist` logo apos instalar Torch 2.5.1. Agora o runtime ML e validado em subprocesso Python limpo e, quando a instalacao passa, o runner reinicia a si mesmo uma vez (`SUPER_VOZ_ML_RUNTIME_REEXECED=1`) antes de continuar. O log esperado e `Runtime ML atualizado; reiniciando o runner para recarregar Torch/Torchaudio/Torchvision`.
